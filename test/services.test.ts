...
import { VnasData, VnasDataCatalog, VnasDataLive, viaProxy } from '../src/services/vnasData'
...
describe('VnasData catalog layer', () => {
  const http = HttpTextFromRecord({ 'catalog/index.json': indexText, 'catalog/airports/MSP.json': mspText })
  const layer = VnasDataCatalog().pipe((l) => l, (l) => l)
  const runWith = <A, E>(e: Effect.Effect<A, E, VnasData>) => Effect.runPromise(e.pipe(Effect.provide(layer), Effect.provide(http)))

  test('decodes the index and an airport, and finds inline scenarios', async () => {
    const out = await runWith(
      Effect.gen(function* () {
        const data = yield* VnasData
        const index = yield* data.index
        const airport = yield* data.airport('MSP', 'ZMP')
        const scenario = yield* data.scenario(airport, airport.scen[0]!.id)
        return { source: data.source, artccs: index.artccs.length, id: airport.id, scenarios: airport.scen.length, first: scenario.name }
      }),
    )
    expect(out).toEqual({ source: 'catalog', artccs: 1, id: 'MSP', scenarios: 64, first: 'KMSP 12s/17 SLCL 5MIT' })
  })

  test('reports missing files and bad shapes as DataError', async () => {
    const missing = await runWith(Effect.gen(function* () { return yield* (yield* VnasData).airport('ZZZ', 'ZMP') }).pipe(Effect.flip))
    ...
    const shape = await Effect.runPromise(
      Effect.gen(function* () { return yield* (yield* VnasData).index }).pipe(Effect.flip, Effect.provide(VnasDataCatalog()), Effect.provide(bad)),
    )
    expect(shape._tag).toBe('DataError')
  })
})

describe('VnasData live layer', () => {
  ...
  const runWith = <A, E>(e: Effect.Effect<A, E, VnasData>) => Effect.runPromise(e.pipe(Effect.provide(VnasDataLive(proxy)), Effect.provide(http)))
  ...
  test('builds the index and the airport file from vNAS documents and resolves scenarios lazily', async () => {
    const out = await runWith(
      Effect.gen(function* () {
        ...
        const index = yield* data.index
        const airport = yield* data.airport('MSP', 'ZMP')
        const scenario = yield* data.scenario(airport, 's1')
        return { source: data.source, index, airport, scenario }
      }),
    )
  ...
  test('proxy templates accept a {url} placeholder or a prefix', () => {
