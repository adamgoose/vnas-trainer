...
describe('command parser', () => {
  ...
  test('ground commands', () => {
    ...
    expect(parsed('RWY 30l TAXI d a HS 12R').command).toEqual(AtcCommand.Runway({ runway: '30L', at: null, via: ['D', 'A'], cross: [], holdShort: '12R' }))
    expect(parsed('RWY 30L d a').command).toEqual(AtcCommand.Runway({ runway: '30L', at: null, via: ['D', 'A'], cross: [], holdShort: null }))
    expect(parsed('RWY 17 TAXI a CROSS 4 12r').command).toEqual(AtcCommand.Runway({ runway: '17', at: null, via: ['A'], cross: ['4', '12R'], holdShort: null }))
    expect(parsed('RWY 17 CROSS 4 HS 12R').command).toEqual(AtcCommand.Runway({ runway: '17', at: null, via: [], cross: ['4'], holdShort: '12R' }))
    expect(parsed('RWY 30L AT d TAXI a').command).toEqual(AtcCommand.Runway({ runway: '30L', at: 'D', via: ['A'], cross: [], holdShort: null }))
    expect(parsed('RWY 30L TAXI a at d cross 4').command).toEqual(AtcCommand.Runway({ runway: '30L', at: 'D', via: ['A'], cross: ['4'], holdShort: null }))
    expect(parseCommandLine(world, null, 'RWY 30L AT')).toMatchObject({ _tag: 'Invalid', error: 'at which taxiway?' })
  ...
  test('tower commands', () => {
    expect(parsed('LUAW').command).toEqual(AtcCommand.LineUpAndWait({ at: null }))
    expect(parsed('CTO').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: null, turn: null, at: null }))
    expect(parsed('CTO 270').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 270, turn: null, at: null }))
    expect(parsed('CTO L 270').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 270, turn: 'L', at: null }))
    expect(parsed('CTO r 090').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 90, turn: 'R', at: null }))
    expect(parsed('CTO TL 270').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 270, turn: 'L', at: null }))
    expect(parsed('CTO FH 360').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 360, turn: null, at: null }))
    expect(parsed('LUAW at d').command).toEqual(AtcCommand.LineUpAndWait({ at: 'D' }))
    expect(parsed('CTO AT d').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: null, turn: null, at: 'D' }))
    expect(parsed('CTO L 250 AT d').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 250, turn: 'L', at: 'D' }))
    expect(parsed('CTO AT d R 090').command).toEqual(AtcCommand.ClearedForTakeoff({ heading: 90, turn: 'R', at: 'D' }))
    expect(parseCommandLine(world, null, 'CTO AT')).toMatchObject({ _tag: 'Invalid', error: 'at which taxiway?' })
    expect(parseCommandLine(world, null, 'LUAW AT')).toMatchObject({ _tag: 'Invalid', error: 'at which taxiway?' })
    ...
    expect(parsed('EXIT').command).toEqual(AtcCommand.Exit({ taxiway: null }))
    expect(parsed('EXIT A5').command).toEqual(AtcCommand.Exit({ taxiway: 'A5' }))
