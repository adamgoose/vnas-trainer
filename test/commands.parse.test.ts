...
describe('command parser', () => {
  ...
  test('ground commands', () => {
    ...
    expect(parsed('TAXI d w w3 HS a3').command).toEqual(AtcCommand.Taxi({ via: ['D', 'W', 'W3'], holdShort: 'A3' }))
    expect(parsed('TAXI A e16').command).toEqual(AtcCommand.Taxi({ via: ['A', 'E16'], holdShort: null }))
    expect(parseCommandLine(world, null, 'TAXI')).toEqual({ _tag: 'Invalid', callsign: null, error: 'taxi where?' })
    expect(parsed('RWY 30l TAXI d a HS 12R').command).toEqual(AtcCommand.Runway({ runway: '30L', via: ['D', 'A'], holdShort: '12R' }))
    expect(parsed('RWY 30L d a').command).toEqual(AtcCommand.Runway({ runway: '30L', via: ['D', 'A'], holdShort: null }))
  ...
  test('tower commands', () => {
    ...
    expect(parsed('CTO').command).toEqual(AtcCommand.ClearedForTakeoff())
