...
- Ground: "push back approved" → PUSH; "push back approved, tail east onto alpha" → PUSH A; "runway three zero left, taxi via quebec, charlie" → RWY 30L TAXI Q C; "taxi to gate echo one six via bravo" → TAXI B E16; "hold short of runway one two right" as part of a taxi → append HS 12R to that taxi command, on its own → HS 12R; "cross runway four" as part of a taxi → append CROSS 4 to that taxi command (several runways: CROSS 4 12R), on its own → CROSS 4; "continue taxi" / "resume" → RES; "hold position" / "stop" → HOLD; "give way to the Delta seven thirty-seven" → GIVEWAY <that callsign>; "expedite" → BREAK; "monitor tower" / "contact ground" → no command, readback only.
