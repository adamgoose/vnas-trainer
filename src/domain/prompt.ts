...
export const COMMAND_REFERENCE = `PUSH [taxiway] | TAXI <taxiways...> [CROSS <rwy...>] [HS <pt>] | RWY <runway> [AT <taxiway>] TAXI <taxiways...> [CROSS <rwy...>] [HS <pt>] |
HS <pt> | CROSS [rwy] | RES | HOLD | BREAK | GIVEWAY <callsign> | LUAW [AT <taxiway>] | CTO [L|R] [hdg] [AT <taxiway>] | EXIT [<taxiway>] |
...
CAPP <rwy> (cleared ILS/approach) | CT (contact tower) | CA (contact approach)`
...
- Ground: "push back approved" → PUSH; "push back approved, tail east onto alpha" → PUSH A; "runway three zero left, taxi via quebec, charlie" → RWY 30L TAXI Q C; an intersection departure, "runway three zero left at delta, taxi via alpha" → RWY 30L AT D TAXI A (AT names the taxiway the runway is entered from; leave it out for full length); "taxi to gate echo one six via bravo" → TAXI B E16; "taxi via quebec, charlie" with no runway named → TAXI Q C (continues to the runway already assigned; if none is assigned yet, use RWY); "hold short of runway one two right" as part of a taxi → append HS 12R to that taxi command, on its own → HS 12R; "cross runway four" as part of a taxi → append CROSS 4 to that taxi command (several runways: CROSS 4 12R), on its own → CROSS 4; "continue taxi" / "resume" → RES; "hold position" / "stop" → HOLD; "give way to the Delta seven thirty-seven" → GIVEWAY <that callsign>; "expedite" → BREAK; "monitor tower" / "contact ground" → no command, readback only.
...
- Center: "climb and maintain flight level three five zero" → CM FL350; "descend and maintain flight level two four zero" → DM FL240; "proceed direct" → DCT <fix>; "contact Minneapolis Approach one two six point seven" → CA; "contact Minneapolis Center one three three point four" (the next sector) → CD.
- Tower: "line up and wait" → LUAW; "runway three zero left at delta, line up and wait" → LUAW AT D; "cleared for takeoff" → CTO; "runway three zero left at delta, cleared for takeoff" → CTO AT D; "turn left heading two five zero, runway three zero left, cleared for takeoff" → CTO L 250; "fly heading two seven zero, cleared for takeoff" → CTO 270; "cleared to land" → CTL; "go around" → GA; "fly heading zero niner zero" → FH 090; "turn left/right heading two seven zero" → TL 270 / TR 270; "climb and maintain five thousand" → CM 5000; "climb and maintain flight level two three zero" → CM FL230; "contact departure" → CD; "exit at alpha five" / "turn off at alpha five" → EXIT A5 (works on final and on the landing roll).
